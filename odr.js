window.addEventListener('load', function () {
    ...
    var s = 10; // scale (in HTML the canvas size is 280px, mapping canvas
    ...
    function canvasPencil() {
        ...
        this.mousemove = function (evt) {
            ...
            dimg[dx][dy] = 1; // set the same pixel on 2D array
             ...
             // not a thin line, but a bold line, like the original digit
                               // drawings...
    ...
    drawCanvasCell(dx - 1, dy - 1);
             digit drawings...
    // not a thin line, but a bold line, like the original
    dimg[dx][dy + 1] =1; dimg[dx][dy - 1] =1; dimg[dx + 1][dy + 1] =1; dimg[dx + 1][dy] =1; dimg[dx + 1][dy - 1] =1;
          var mouseX = 0;
...
function clearCanvas() { canvas.width = canvas.width; drawCanvasGrid();
for (var i = 0; i < ih; i++) {
for (var j = 0; j < iw; j++) {
            dimg[i][j] = 0;
        }
...
= 1; = 1;
              ...
              function drawCanvasCell(x, y) { context.fillRect(x * s, y * s, s, s);
}
function drawCanvasGrid() {
   for
}
for
(varx=0;x<=iw;x+=1){ context.moveTo(x * s, 0); context.lineTo(x * s, ih * s);
    (vary=0;y<=ih;y+=1){
 context.moveTo(0, y
context.lineTo(iw *
* s); s,y*s);
...
function callWebService() { var dimgarray = "" for(vari=0;i<ih;
for(varj=0;
      dimg[dx - 1][dy + 1]    = 1;
...
"f752", "f753", "f754", "f755", "f756", "f757", "f758", "f759", "f760",
"f761", "f762", "f763", "f764", "f765", "f766", "f767", "f768", "f769",
...
jQuery.ajax({
url: "<https://odr-test234.azure-api.net/predict>",
beforeSend: function (xhrObj) {
xhrObj.setRequestHeader("Ocp-Apim-Subscription-Key", "<59c50395f6c146228898dacbc63a424c>");
